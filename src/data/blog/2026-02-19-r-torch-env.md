...
本指南适用于 Windows、Ubuntu 及 macOS 环境。

**1. 安装 R 基础环境**

首先，请前往 CRAN 镜像站（推荐使用[清华大学 TUNA 镜像](https://mirrors.tuna.tsinghua.edu.cn/CRAN/)）下载并安装适用于你操作系统的 R 运行环境。该镜像站提供了 Linux、macOS 和 Windows 三大平台的安装包，请按需选择。


**2. 安装 torch 核心包**

R 环境配置完成后，你可以通过以下两种方式安装 [torch](https://github.com/mlverse/torch) 包：
...
注意：执行上述命令时，R 会自动安装 `R6`、`Rcpp`、`coro` 等十余个依赖包。在网络状况良好的情况下，此过程通常仅需数十秒。
...
- 下载 Libtorch：从 pytorch.org 获取 CPU 版本的 Libtorch 核心库（约 200MB，视操作系统而定）。
- 下载 Lantern：从 torch-cdn.mlverse.org 获取 Lantern 辅助库（约 3MB）。

由于底层依赖库（Libtorch）服务器位于海外，下载速度可能较慢。若自动下载失败，建议开启网络代理或参考[torch 的官网文档](https://torch.mlverse.org/docs/articles/installation.html)进行手动离线安装。
